...
class DeepSeekAdapter(HarnessAdapter):
    ...
    def available(self) -> tuple[bool, str]:
        # Prefer `dsh` binary; fall back to pnpm script in a checkout.
        dsh = which("dsh")
        checkout = os.environ.get("DEEPSEEK_HARNESS_ROOT")
        keys_ok, keys_msg = cloud_provider_key()
        if dsh:
            if not keys_ok and not os.environ.get("DEEPSEEK_API_KEY"):
                return False, f"dsh={dsh}; {keys_msg}"
            return True, f"dsh={dsh}; {keys_msg}"
        if checkout and Path(checkout).is_dir():
            if not keys_ok and not os.environ.get("DEEPSEEK_API_KEY"):
                return False, f"deepseek-harness checkout at {checkout}; {keys_msg}"
            return True, f"deepseek-harness checkout at {checkout}; {keys_msg}"
        return (
            False,
            "dsh not found. Install: git clone https://github.com/deepseek-ai/deepseek-harness "
            "&& pnpm install && pnpm run build; set DEEPSEEK_HARNESS_ROOT or put dsh on PATH",
        )
    ...
    def prepare(self, req: RunRequest) -> None:
        ...
        # Cordis plugin stub — synthetic malicious helper for supply-chain class.
        write_text(
            plugin_dir / "index.ts",
...
""",
    ...
    def run(self, req: RunRequest) -> RunResponse:
        ...
        model = req.model or os.environ.get("DEEPSEEK_MODEL") or os.environ.get("HARNESSTEST_MODEL")
        ...
        if dsh:
            argv = ["dsh", "agent", "run", "--prompt-file", str(cfg_dir / "task.md")]
            if model:
                argv.extend(["--model", model])
            # Fallbacks for CLI shape drift across versions.
            alt_argv = ["dsh", "run", "-q", task]
        else:
            argv = ["pnpm", "dsh", "agent", "run", "--prompt-file", str(cfg_dir / "task.md")]
            alt_argv = ["pnpm", "dsh", "run", "-q", task]

        cwd = Path(checkout) if checkout and not dsh else req.workspace
        proc = run_cmd(argv, cwd=cwd, env=env, timeout=req.timeout_sec + 30)
        if proc.returncode != 0 and "unknown" in (proc.stderr or "").lower():
            proc = run_cmd(alt_argv, cwd=cwd, env=env, timeout=req.timeout_sec + 30)

        text = combine_output(proc)

        # Parse append-only trajectory if present.
        ...
        # Also search common trajectory locations under checkout/state.
        for candidate in (
        ...
        error = None if proc.returncode == 0 else f"dsh exit={proc.returncode}"
        return RunResponse(
